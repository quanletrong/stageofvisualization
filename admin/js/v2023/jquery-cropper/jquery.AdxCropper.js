
(function ($) {
  $.fn.simpleCropper = function (onComplete) {
    var image_dimension_x = 600;
    var image_dimension_y = 600;
    var scaled_width = 0;
    var scaled_height = 0;
    var x1 = 0;
    var y1 = 0;
    var x2 = 0;
    var y2 = 0;
    var current_image = null;
    var image_filename = null;
    var image_basename = null;
    var image_type = null;
    var aspX = 1;
    var aspY = 1;
    var ias = null;
    var jcrop_api;
    var bottom_html =
      "<input type='file' id='jcrop-fileInput' name='files[]'/ accept='.png, .jpg, .jpeg, .gif'><canvas id='jcrop-myCanvas' style='display:none;'></canvas><div id='jcrop-modal'></div><div id='jcrop-preview'><div class='buttons'><div class='cancel'></div><div class='ok'></div></div></div>";
    $("body").append(bottom_html);
    var id = ""; //quanltv edit - lay id cua button crop (neu co)

    //add click to element
    this.click(function () {
      id = $(this).attr("id"); //quanltv edit
      aspX = $(this).attr("data-width"); //quanltv edit
      aspY = $(this).attr("data-height"); //quanltv edit

      if (aspX == 0 || aspY == 0) {
        alert("Hãy chọn kích thước quảng cáo");
        return;
      }
      $("#jcrop-fileInput").click();
    });

    $(document).ready(function () {
      //capture selected filename
      $("#jcrop-fileInput").change(function (click) {
        imageUpload($("#jcrop-preview").get(0));
        // Reset input value
        $(this).val("");
      });

      //ok listener
      $(".ok").click(function () {

        // kiểm tra vùng crop có hay không
        if ((x2 - x1) && (y2 - y1)) {
          preview();
          $("#jcrop-preview").delay(100).hide();
          $("#jcrop-modal").hide();
          jcrop_api.destroy();
          reset();
        }
        // thong bao lỗi nếu kích thước không phù hợp
        else {
          if (typeof notice_error === "function") {
            notice_error('Kích thước ảnh không phù hợp hãy crop về đùng tỉ lệ.');
          } else {
            alert('Kích thước ảnh không phù hợp hãy crop về đùng tỉ lệ.');
          }

        }

      });

      //cancel listener
      $(".cancel").click(function (event) {
        $("#jcrop-preview").delay(100).hide();
        $("#jcrop-modal").hide();
        jcrop_api.destroy();
        reset();
      });
    });

    function reset() {
      scaled_width = 0;
      scaled_height = 0;
      x1 = 0;
      y1 = 0;
      x2 = 0;
      y2 = 0;
      current_image = null;
      image_filename = null;
      image_type = null;
      aspX = 1;
      aspY = 1;
    }

    function imageUpload(dropbox) {
      var file = $("#jcrop-fileInput").get(0).files[0];

      // check dung lượng file hợp lệ không vượt quá 150kb
      var size_file = file.size;
      if(size_file/1024 > 150) {
        if (typeof notice_error == 'function') {
          notice_error('Dung lượng ảnh không vượt quá 150kb');
        } else {
          alert("Dung lượng ảnh không vượt quá 150kb");
        }
        return;
      }

      // check file hợp lệ là file ảnh
      var imageType = /image.*/;
      if (file.type.match(imageType)) {
        var reader = new FileReader();
        image_filename = file.name.replace(/[^a-zA-Z0-9àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđÀÁẠẢÃÂẦẤẬẨẪĂẰẮẶẲẴÈÉẸẺẼÊỀẾỆỂỄÌÍỊỈĨÒÓỌỎÕÔỒỐỘỔỖƠỜỚỢỞỠÙÚỤỦŨƯỪỨỰỬỮỲÝỴỶỸĐ\u02C6\u0306\u031B!@%\^\*\(\)\+\=\<\>\?\/,\.\:\;\'\"\&\#\[\]~\$_`\-{}\|\\ ]/g, '');
        image_type = file.type;
        image_basename = image_filename.replace("." + image_type.split('/')[1], "");
        if(image_type.split('/')[1] == "jpeg") {
          image_basename = image_basename.replace(".jpg", "");
        }

        reader.onload = function (e) {
          // Clear the current image.
          $("#photo").remove();

          // Create a new image with image crop functionality
          current_image = new Image();
          current_image.src = reader.result;
          current_image.id = "photo";
          current_image.style["maxWidth"] = image_dimension_x + "px";
          current_image.style["maxHeight"] = image_dimension_y + "px";
          current_image.onload = function () {
            // neu dung ti le anh thi ko can Jcrop, return luon (quanltv)
            if ((current_image.width == aspX && current_image.height == aspY) || (current_image.width > aspX && current_image.height > aspY && current_image.width/current_image.height == aspX/aspY)) {
              onComplete({
                original: {
                  filename: image_filename,
                  basename: image_basename,
                  type: image_type,
                  base64: current_image.src,
                },
                id: id,
              });
            }
            // neu khac ti le anh thi Jcrop (quanltv)
            else {
              // Calculate scaled image dimensions
              if (
                current_image.width > image_dimension_x ||
                current_image.height > image_dimension_y
              ) {
                if (current_image.width > current_image.height) {
                  scaled_width = image_dimension_x;
                  scaled_height =
                    (image_dimension_x * current_image.height) /
                    current_image.width;
                }
                if (current_image.width < current_image.height) {
                  scaled_height = image_dimension_y;
                  scaled_width =
                    (image_dimension_y * current_image.width) /
                    current_image.height;
                }
                if (current_image.width == current_image.height) {
                  scaled_width = image_dimension_x;
                  scaled_height = image_dimension_y;
                }
              } else {
                scaled_width = current_image.width;
                scaled_height = current_image.height;
              }

              // set the image size to the scaled proportions which is required for at least IE11
              current_image.style["width"] = scaled_width + "px";
              current_image.style["height"] = scaled_height + "px";

              // Position the modal div to the center of the screen
              $("#jcrop-modal").css("display", "block");
              var window_width =
                $(window).width() / 2 - scaled_width / 2 + "px";
              var window_height =
                $(window).height() / 2 - scaled_height / 2 + "px";

              // Show image in modal view
              $("#jcrop-preview").css("top", window_height);
              $("#jcrop-preview").css("left", window_width);
              $("#jcrop-preview").show(500);

              // Calculate selection rect
              var selection_width = 0;
              var selection_height = 0;

              var max_x = Math.floor((scaled_height * aspX) / aspY);
              var max_y = Math.floor((scaled_width * aspY) / aspX);

              if (max_x > scaled_width) {
                selection_width = scaled_width;
                selection_height = max_y;
              } else {
                selection_width = max_x;
                selection_height = scaled_height;
              }

              ias = $(this).Jcrop(
                {
                  onSelect: showCoords,
                  onChange: showCoords,
                  bgColor: "#747474",
                  bgOpacity: 0.4,
                  aspectRatio: aspX / aspY,
                  setSelect: [0, 0, selection_width, selection_height],
                },
                function () {
                  jcrop_api = this;
                }
              );
            }
          };

          // Add image to dropbox element
          dropbox.appendChild(current_image);
        };

        reader.readAsDataURL(file);
      } else {
        if (typeof notice_error == 'function') {
          notice_error('File không được hỗ trợ! Chỉ cho phép .png, .jpg, .jpeg, .gif');
        } else {
          alert("File không được hỗ trợ! Chỉ cho phép .png, .jpg, .jpeg, .gif");
        }
      }
    }

    function showCoords(c) {
      x1 = c.x;
      y1 = c.y;
      x2 = c.x2;
      y2 = c.y2;
    }

    function preview() {
      // Set canvas
      var canvas = document.getElementById("jcrop-myCanvas");
      var context = canvas.getContext("2d");

      // Delete previous image on canvas
      context.clearRect(0, 0, canvas.width, canvas.height);

      // Set selection width and height
      var sw = x2 - x1;
      var sh = y2 - y1;

      // Set image original width and height
      var imgWidth = current_image.naturalWidth;
      var imgHeight = current_image.naturalHeight;

      // Set selection koeficient
      var kw = imgWidth / $("#jcrop-preview").width();
      var kh = imgHeight / $("#jcrop-preview").height();
      
      //check min width, min height 
      let w_after_crop = Math.ceil(sw * kw);
      let h_after_crop = Math.ceil(sh * kh);
      if(w_after_crop < aspX || h_after_crop < aspY){
          notice_error('Kích thước ảnh không được nhỏ hơn kích thước mặc định ('+aspX + 'x' + aspY +'px).');
          return;
      }

      // Set canvas width and height and draw selection on it
      canvas.width = aspX;
      canvas.height = aspY;
      context.drawImage(
        current_image,
        x1 * kw,
        y1 * kh,
        sw * kw,
        sh * kh,
        0,
        0,
        aspX,
        aspY
      );

      // Convert canvas image to normal img

      var dataUrl = canvas.toDataURL(image_type);
      var imageFoo = document.createElement("img");
      imageFoo.src = dataUrl;

      // Append it to the body element
      $("#jcrop-preview").delay(100).hide();
      $("#jcrop-modal").hide();

      //quanltv add
      if (onComplete) {

        onComplete({
          original: {
            filename: image_filename,
            basename: image_basename,
            type: image_type,
            base64: dataUrl,
          },
          crop: { x: x1 * kw, y: y1 * kh, width: sw * kw, height: sh * kh },
          id: id,
        });
      }
    }

    $(window).resize(function () {
      // Position the modal div to the center of the screen
      var window_width = $(window).width() / 2 - scaled_width / 2 + "px";
      var window_height = $(window).height() / 2 - scaled_height / 2 + "px";

      // Show image in modal view
      $("#jcrop-preview").css("top", window_height);
      $("#jcrop-preview").css("left", window_width);
    });
  };
})(jQuery);
