...
class Spreadsheet
{
    ...
    public function setNumberByColAndRow($col, $row, $number)
    {
        //reset number
        if($number != 'N/A') {
        ...
        $this->setActiveSheetIndex(0)->setCellValueByColumnAndRow($col, $row, $number);
        $colName = $this->Numeric2Alphabet($col);
        $this->setActiveSheetIndex(0)->getStyle($colName.$row)->getNumberFormat()->setFormatCode('#,##0');
    }

    public function setPercentByColAndRow($col, $row, $number)
    {
        //reset percent
        if($number != 'N/A') {
            $number = str_replace(",", "", $number);
            $number = is_numeric($number) ? $number / 100 : 0;
        }
        ...
        if ($number < 0.00001) {
            ...
            $this->setActiveSheetIndex(0)->getStyle($colName.$row)->getNumberFormat()->setFormatCode('#,##0.000%');
        }
    }
}
