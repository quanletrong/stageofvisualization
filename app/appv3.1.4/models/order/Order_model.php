...
{
    private $_status_sort = [ORDER_PENDING, ORDER_QC_CHECK, ORDER_AVAIABLE, ORDER_PROGRESS, ORDER_REWORK, ORDER_DONE, ORDER_DELIVERED, ORDER_COMPLETE, ORDER_CANCLE];
    ...
    function add_order($id_style, $create_time, $id_user, $status, $order_type, $create_id_user, $ed_type)
    {
        ...
        $sql = "INSERT INTO tbl_order (id_style, create_time, id_user, status, order_type, create_id_user, ed_type, expire) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $iconn->prepare($sql);
        if ($stmt) {
            $param = [$id_style, $create_time, $id_user, $status, $order_type, $create_id_user, $ed_type, $expire];
