// for cpc adx , not change
const max_title_set_style = 4;
const max_body_set_style  = 24;
const maxBStyleBold = maxBStyleColor = 12;
// jquery.caret 1.0.2
...
//for country
function toggle_all_coutry()
{
    var checked =  $("#check_all_country").prop('checked');
    $(".countrys").prop('checked',checked);
    $(".provinces").prop('checked',checked);
    count_select_only_area();
}

function count_select_only_area()
{

    $(".title_coutry").each(function(){

        var num;
        var title = $(this).data('title');

        var num_country = $(".num_ct_"+title).length;
        var num_country_checked = $(".num_ct_"+title+":checked").length;

        var num_prov_checked = $(".num_child_ct_"+title+":checked").length;

        if(num_country == num_country_checked)
        {
            num = 'All';
        }
        else
        {
            num = num_prov_checked + num_country_checked;
        }

        if(parseInt(num) > 0 || num == 'All')
        {
            $(".count_ct_"+title).text(num + ' selected');
        }
        else
        {
            $(".count_ct_"+title).text('');
        }
...
{
    var location = $('#chkBac').prop('checked') || $('#chkTrung').prop('checked') || $('#chkNam').prop('checked');

    var chkCity = ($.trim($('#trgCity').val()) != '');
    var chkCountry = true;

    if($("#slcCountry").length > 0)
    {
        var chkCountry = $("#slcCountry").val() != null && $("#slcCountry").val() != "" ? true : false;
    }

    if(!location && !chkCity && !chkCountry)
    {
        $('#errorLocation').show();
    }
    else
...
{
    var num;
    var num_country = $(".countrys").length;
    var num_country_checked = $(".countrys:checked").length;
    var num_prov_checked = $(".provinces:checked").length;
...
function is_set_all_country()
{
    var num_prov = $(".countrys").length;
    var num_prov_checked = $(".countrys:checked").length;
    if(num_prov == num_prov_checked && num_prov_checked > 0)
    {
        $("#check_all_country").prop("checked", true);
    }
    else
    {
        $("#check_all_country").prop("checked", false);
    }
}
...
{
    var str = "";
    $(".lstcountrygetid:checked").each(function () {
        var id = $(this).val();
        str += str == "" ? id : ","+id;
    });
    return str;
...
function select_coutry(id,type)
{
    is_coutry_select(id,type);
    is_set_all_country();
}

function is_coutry_select(id, type)
{
    if(type == 1)
    {
        var checked =  $("#country_"+id).prop('checked');
        $(".ctprov_"+id).prop('checked',checked);

    }
    if(type == 2)
    {

        var num_channel = $(".ctprov_"+id).length;
        var num_checked = $(".ctprov_"+id+":checked").length;

        if(num_checked == num_channel) {
            $("#country_" + id).prop('checked', true);
        }
        else
        {
            $("#country_" + id).prop('checked', false);
        }
    }
...
function tracking_url_view(id, eid)
{
    if($(id).is(':checked'))
    {
        //$(eid).removeAttr('disabled').removeClass('readonly');
        $("#trackurl").next().css('opacity','1');
        $("#trackurl").next().find(".ui-autocomplete-input").prop('disabled', false);
    }
    else
    {
        //$(eid).attr('disabled', 'disabled').addClass('readonly').val('');
        $("#trackurl").next().find(".ui-autocomplete-input").prop('disabled', true).val('');
        $("#trackurl").next().find(".tagit-choice").remove();
        $("#trackurl").next().css('opacity','.2');
        $("#trackurl").val("");
        $("#errorTrackViewUrl").hide();
    }
}

function step1_ptag_change(eid, reviewId, trptagId)
{
    var tmp = $(eid).val();
    if (Encoder.hasEncoded(tmp)) {
        tmp = Encoder.htmlDecode(tmp);
    }
    if (chkHtmlTags(tmp)) {
        tmp = stripHtmlTags(tmp);
        $(eid).val(tmp);
    }
    ...
    var txt = $.trim($(eid).val());
    if (Encoder.hasEncoded(txt)) {
        txt = Encoder.htmlDecode(txt);
    }
    if (chkHtmlTags(txt)) {
...
function updateGAUrl(checkbox_GA = '#chkTrackGA', input_url = '#desUrl'){
    
    var url = $.trim($(input_url).val());

    var arr_utm = ['utm_source=Admicro_AdX', 'utm_campaign=AdX', 'utm_content=[admdomain]'];

    //var text_search = 'utm_source=Admicro_AdX&utm_campaign=AdX&utm_content=[admdomain]';

    var isGA = $(checkbox_GA).is(':checked') ? true : false;
    if(url != ''){
        if(isGA){
            url = get_utm(arr_utm, url, 'add');
        }else{

            url = get_utm(arr_utm, url, 'remove');


        }

        $(input_url).val($.trim(url));
    }
}

function get_utm(arr_utm, url , type){
    var text_search = 'utm_source=Admicro_AdX&utm_campaign=AdX&utm_content=[admdomain]';
    var arr_url = url.split("?");

    if(type == 'add'){
        if(arr_url.length == 1 || arr_url[1] == ""){
...
function step1_banname_change(eId)
{
    var tmp = $(eId).val();
    var txtscript = $.trim($('#txtscript').val());
    if (Encoder.hasEncoded(tmp)) {
        tmp = Encoder.htmlDecode(tmp);
    }
    if (chkHtmlTags(tmp)) {
    ...
    if(tex == '')
    {
        step1_banname_error(true);
        $('#step1RevTitle').html('');
        $('#review_title').html('');
    }
    else
    {
        step1_banname_error(false);
        $('#step1RevTitle').html(tex);
        $('#review_title').html(tex);
    }
}

function step1_banname_error(isShow)
{
    if(isShow)
    {
...
{
    var isShow = (!(typeof(eIdShow) === 'undefined' || eIdShow === ''));
    var errEId = (typeof(errEId) === 'undefined' || errEId === '') ? '' : errEId;
    var chk = true;
    var hasEvt = false;
    var type = eIdCheck == '#title' ? 't' : 'b';
    ...
    if (chk) {
        ...
        if (len > countLimit) {
            ...
            if (type == 't') {
                ...
                $('#tStyleMax').html(styleMax);

            }
            else {
                $('#step1_suggest_body_160').html(tex);
                $('#review_bodytext').html(tex);
                $('#bStyleMax').html(styleMax);
            }
            if (isShow) {
                $(eIdShow).html(tex);
            }
...
function step1_desUrlKeyUp(eId, urlChkDomain)
{
    ...
    urlChkDomain = (typeof(urlChkDomain) === 'undefined' || urlChkDomain === '') ? '' : urlChkDomain;
    var tex = $.trim($(eId).val());
    if (Encoder.hasEncoded(tex)) {
        tex = Encoder.htmlDecode(tex);
    }
    ...
    if(tex == '')
    {
        ...
        $('#review_url').html('');
        $('#step1LinkImg').attr('href', 'javascript:void(0);');
        $('#step1LinkImgReplace').attr('href', 'javascript:void(0);');
        $('#review_image_replace_link').attr('href', 'javascript:void(0);');
    }
    else
    {
        if(!is_url_banner(tex))
        {
            step1ErrorDesUrl(true);
            $('#review_url').html('');
            $('#step1LinkImg').attr('href', 'javascript:void(0);');
        ...
        else
        {
            ...
            $('#review_url').html(tex);
            $('#step1LinkImg').attr('href', tex);
            $('#step1LinkImgReplace').attr('href', tex);
            $('#review_image_replace_link').attr('href', tex);



            if(urlChkDomain != '')
            {
                user_website_check_domain(urlChkDomain);
            }
        }
    }
    checkGAUrl();
...
function user_website_check_domain(url){
    var urlx = '';
    urlx = $.trim($('#desUrl').val());
    if (urlx != ''){
        var strdomain = getDomainFromUrl(urlx);
        var _onSuccess = function(data)
        {
            ...
            if(data==1){
                chkUserWeb('');
                $('#visitor_').attr('disabled', false);
                $('#visitor_1').attr('disabled', false);
                $('#visitor_2').attr('disabled', false);
                // khong cho phep chon
            }else{
                // reset ve default
                chkUserWeb('');
                $('#visitor_').attr('disabled', 'disabled');
                $('#visitor_1').attr('disabled', 'disabled');
                $('#visitor_2').attr('disabled', 'disabled');
            }
        };
        var param = {'ajax' : '1', 'strdomain': strdomain};
...
function checkGAUrl()
{
    var desUrl = $.trim($('#banner-url').val());
    if(desUrl != '')
    {
        var lower_url = desUrl.toLowerCase();
        
  
        var arr_utm = ['utm_source=admicro_adx', 'utm_campaign=adx', 'utm_content=[admdomain]'];
        var arr_utm_corect = {
            'utm_source=admicro_adx' : 'utm_source=Admicro_AdX',
            'utm_campaign=adx' : 'utm_campaign=AdX',
            'utm_content=[admdomain]' : 'utm_content=[admdomain]'
        };
        
        $('#banner-trackingGA').prop('checked', true);
        
        arr_utm.forEach(function(val){
            if(lower_url.indexOf(val) < 0){
                 console.log("1");
               $('#banner-trackingGA').prop('checked', false);
            } else{
                desUrl = desUrl.replace(val, arr_utm_corect[val]);
                $('#banner-url').val($.trim(desUrl));
            }
...
// step 2 website visitor user
function chkUserWeb(val)
{
    $('#visitor_' + val).prop('checked', true);
    $('#hddVisitor').val(val);
    var id = '#trgUwebName_' + val;
    var strUweb = '<li>' + $.trim($(id).html()) + '</li>';

    $('#review_uweb').html(strUweb);
    if(strUweb != '')
    {
        $('#trRevUserWeb').css('display', '');
    }
    else
    {
        $('#trRevUserWeb').css('display', 'none');
    }
}
// end step 2 website visitor user

function step1_desUrlKeyUp2(eId, errEId) {
    var tex = $.trim($(eId).val());
    if (Encoder.hasEncoded(tex)) {
        tex = Encoder.htmlDecode(tex);
    }
    ...
    if (tex == '') {
        ...
        if (!is_url_banner(tex)) {
            ...
            $('#step1_domain_160').html($('#desUrl').val());
            $('#review_url').html($('#desUrl').val());
        }
        else {
            step1ErrorDesUrl(false);
            $('#step1_domain').html(getDomainFromUrl($('#desUrl').val()));
            $('#step1_domain_160').html(getDomainFromUrl($('#desUrl').val()));
...
function step1_script_change()
{
    var check = false;
    var script = $('#txtscript').val().trim();
    var domain_valid = $("#domain_valid_script").val();
    if(script != "")
    {
        if(domain_valid != "" && isUrl(script))
        {
            var arr_domain_valid = JSON.parse(domain_valid);
            var domain_url_script = getDomainFromUrl(script);

            if(arr_domain_valid.indexOf(domain_url_script) != -1)
            {
                var rexHtmlJs = /^(https|http):\/\/[^\s$.?#].[^\s]*(html|js)$/
                var chk = script.match(rexHtmlJs);
                if(chk)
                {
                    check = true;
                }
...
function checkUrlTracking(url)
{
    var regex = /^(https):\/\/([A-Z0-9][A-Z0-9_-]*(?:\.[A-Z0-9][A-Z0-9_-]*)+):?(\d+)?\/?/i;
    if(regex.test(url)) {
        return true;
    }
    return false;
...
// for keyword
// function slcBanTypeChangeKeyword(suggestbid)
// {
//     suggestbid = (typeof(suggestbid) == 'undefined' || suggestbid == '') ? false : suggestbid;
//     var btype = $.trim($('#slcBanType').val());
//     var minCpc = $.trim($('#hddMinCPC').val());
//     minCpc = parseInt(minCpc);
//     var minCpm = $.trim($('#hddMinCPM').val());
//     minCpm = parseInt(minCpm);
//     var currPriceKeyword = $.trim($('#txtBanPriceKeyword').val());
//     currPriceKeyword = currPriceKeyword.replace(/,/gi, '');
//     $('#review_btype').html($('#slcBanType').find(":selected").text());
//     // cpc
//     if(btype == '1')
//     {
//         $('#minPriceNoteKeyword').html(addCommas(minCpc));
//         if(currPriceKeyword <= minCpc)
//         {
//             currPriceKeyword = minCpc;
//         }
//         else
//         {
//             var tmp = currPriceKeyword % 100;
//             var tmp2 = currPriceKeyword - tmp;
//             currPriceKeyword = tmp > 0 ? (tmp2 + 100) : tmp2;
//         }
//         $('#txtBanPriceKeyword').val(addCommas(currPriceKeyword));
//         $('#review_pricebid').html(addCommas(currPriceKeyword));
//     }
//     // cpm
//     else if(btype == '2')
//     {
//         $('#minPriceNoteKeyword').html(addCommas(minCpm));
//         if(currPriceKeyword <= minCpm)
//         {
//             currPriceKeyword = minCpm;
//         }
//         else
//         {
//             var tmp = currPriceKeyword % 100;
...
function step1_bodyText_error(isShow)
{
    if(isShow)
    {
        $('#errorBodyText').show();
        $('#bodyTextSponsor').addClass('error-border');
    }
    else
    {
        $('#errorBodyText').hide();
        $('#bodyTextSponsor').removeClass('error-border');
    }
}

function step1_keyword_error(isShow)
{
    var arrNum = $('#hddKeyword').val();
    ...
    if(numLength > 0)
    {
        ...
        {
            $('#errorKeyword').show();
            $('#errorKeywordstep3').show();
        }
        else
        {
            $('#errorKeyword').hide();
            $('#errorKeywordstep3').hide();
        }
    }
    else
    {
...
function step1ErrorImageReplace(isShow)
{
    if(isShow)
    {
        $('#errorStep1ImageReplace').show();
    }
    else
    {
        $('#errorStep1ImageReplace').hide();
    }
}
...
//--------------------tooltip----------------------
/* tool tip */
function tooltip_detail(e, strTipTitle, strTipBody, width, xOffset, yOffset, isFixWidth)
{
    ...
    {
        $('#tooltips-detail').width(width);

        $("#tooltips-detail-title").html(strTipTitle);
        $("#tooltips-detail-body").html(strTipBody);

        var ttipW = $('#tooltips-detail').width();
        var ttipH = $('#tooltips-detail').height();
        ...
        $("#tooltips-detail").css('z-index','-1000000001');
        $("#tooltips-detail").show();

        var tooltipHeight = $('#tooltips-detail').outerHeight();
        var tooltipWidth = $('#tooltips-detail').outerWidth();

        var ttop = pos.top - yOffset - tooltipHeight;

        var tleft = pos.left - xOffset;
        var arrowPosLeft = xOffset - ($('#tooltips-detail-arrow-down').outerWidth() / 2) + (eWidth / 2);
        ...
        $('#tooltips-detail-arrow-down').css("left", arrowPosLeft + "px");
        $("#tooltips-detail").css("top",(ttop - 10) + "px");
        $("#tooltips-detail").css("left",tleft + "px");
        $("#tooltips-detail").css('z-index','1000000001');
    }
}

function remove_tooltip_detail()
{
    $("#tooltips-detail-title").html('');
    $("#tooltips-detail-body").html('');
    $("#tooltips-detail").hide();
}
/* end tool tip */

function check_between_time()
{
    var startTime = '00:00:00';
    var endTime = '05:59:59';
...
function slcBanTypeChange(suggestbid)
{
    suggestbid = (typeof(suggestbid) == 'undefined' || suggestbid == '') ? false : suggestbid;
    ...
    var minCpm = $.trim($('#hddMinCPM').val());
    minCpm = parseInt(minCpm);
    var currPrice = $.trim($('#txtBanPrice').val());
    currPrice = currPrice.replace(/,/gi, '');
    currPrice = parseInt(currPrice);
    $('#review_btype').html($('#slcBanType').find(":selected").text());
    // cpc
    if(btype == '1')
    {
        $('#minPriceNote').html(addCommas(minCpc));
        if(currPrice <= minCpc)
        {
            currPrice = minCpc;
        }
        ...
        {
            var tmp = currPrice % 100;
            var tmp2 = currPrice - tmp;
            currPrice = tmp > 0 ? (tmp2 + 100) : tmp2;
        }
        $('#txtBanPrice').val(addCommas(currPrice));
        $('#review_pricebid').html(addCommas(currPrice));
    }
    // cpm
    else if(btype == '2')
    {
        $('#minPriceNote').html(addCommas(minCpm));
        if(currPrice <= minCpm)
        {
            currPrice = minCpm;
        }
...
function suggest_bid()
{
    var size_type = $.trim($('#hddBanSize').val());
    var ban_type = $.trim($('#slcBanType').val());
    var str_channel = $.trim($('#hddTrgChannelId').val());

    var bac = ($('#chkBac').prop('checked')) ? '1' : '0';
    var trung = ($('#chkTrung').prop('checked')) ? '2' : '0';
    var nam = ($('#chkNam').prop('checked')) ? '3' : '0';
    var str_location = '';
    str_location += (bac == '1') ? ((str_location != '') ? ','+bac : bac) : '';
    str_location += (trung == '2') ? ((str_location != '') ? ','+trung : trung) : '';
    str_location += (nam == '3') ? ((str_location != '') ? ','+nam : nam) : '';
    ...
    var _onsuccess = function(data)
    {
        ...
        {
            $('#suggest_price').html(data);
        }
    };
    var params = {size_type:size_type, ban_type:ban_type,str_location:str_location,str_channel:str_channel};
    var url = makeSiteUrl('createad/suggest_bid');
    getAjax(url,params, '', 'POST', '', false, _onsuccess);
}