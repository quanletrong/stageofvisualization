...
class AntiUTF8
{
  ...
  public static function rawurldecode($str, $multi_decode = true)
  {
    ...
    $flags = version_compare(PHP_VERSION, '5.4', '>=') ? ENT_QUOTES | ENT_HTML5 : ENT_QUOTES;
  ...
  public static function substr_count($haystack, $needle, $offset = 0, $length = null, $encoding = 'UTF-8', $cleanUtf8 = false)
  {
    ...
    if ($offset || $length !== null) {
      ...
      if (
          ...
          $length + $offset <= 0
          &&
          !version_compare(PHP_VERSION, '7.1', '>=') // output from "substr_count()" have changed in PHP 7.1
      ) {
        return false;
      }
...
class CI_AntiXSS
{
  ...
  private function _do($str)
  {
    ...
    $str = AntiUTF8::clean($str, true, true, false);
  ...
  private function _entity_decode($str)
  {
    ...
    $flags = version_compare(PHP_VERSION, '5.4', '>=') ?
        ENT_QUOTES | ENT_HTML5 | @ENT_DISALLOWED | @ENT_SUBSTITUTE :
        ENT_QUOTES;

    // decode
    if (strpos($str, $this->_xss_hash) !== false) {
      $str = AntiUTF8::html_entity_decode($str, $flags);
    } else {
      $str = AntiUTF8::rawurldecode($str);
    }
  ...
  /**
   * Decode the html-tags via "AntiUTF8::html_entity_decode()" or the string via "AntiUTF8::rawurldecode()".
   *
   * @param string $str
   *
   * @return string
   ...
   * ... however, AntiUTF8::clean() above already strips the
